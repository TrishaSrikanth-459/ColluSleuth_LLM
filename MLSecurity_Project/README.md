...
The system runs decentralized HotpotQA discussions with benign workers, covert malicious workers, and optional trusted detector agents. Detectors use interrogation-based analysis with Reid-style targeted questions and can quarantine suspicious workers through the existing permission manager.
...
Defaults: `25` HotpotQA tasks per condition, `5` repetitions, `4` turns, `1000` tokens per response.
...
Run a tiny validation matrix before the full experiment:

```bash
python parallel_experiment_runner.py --smoke --smoke-tasks 2 --max-concurrent 1 --run-label smoke
```

The smoke test checks a clean baseline, all-malicious sanity conditions for every attack, and one detector condition to exercise interrogation/quarantine.

If Azure filters a detector prompt, the detector falls back to a conservative local evidence check so detector-condition runs remain resumable instead of failing mid-experiment.
